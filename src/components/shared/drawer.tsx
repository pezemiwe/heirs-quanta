...
export const Drawer = ({
  ...
  return createPortal(
    ...
    >
      ...
      />
      <div
        className={`relative z-10 flex h-full flex-col border-l border-border bg-surface shadow-[-12px_0_40px_-12px_rgba(0,0,0,0.18)] ${sizeClasses[size]} animate-slide-in-right`}
      >
        {(title || description) && (
        ...
        {footer && (
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-3 border-t border-border bg-surface-muted px-6 py-4">
            {footer}
          </div>
        )}
      </div>
